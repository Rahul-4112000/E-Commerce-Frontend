...
export const Navbar = () => {
  ...
  const { data } = useApi(getUserProfile, { immediate: true });
  ...
  return (
    <nav className='bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10'>
      <div className='flex items-center gap-2'>
        ...
        <span className='text-xl font-bold bg-clip-text text-transparent from-indigo-600 to-violet-600'>ERTH</span>
      </div>
      <div className='flex items-center gap-4'>
        <button className='flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-gray-100 transition-colors text-gray-700 font-medium cursor-pointer'>
          ...
          <span>{data ? data.data.user.name : 'User'}</span>
        </button>
